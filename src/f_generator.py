...
import pandas as pd
import numpy as np
from utils import create_ym_format, month_diff

def time_travel(df, last_n_months):
    start = (df['last_x_months'] > 1) # plus 1 month distance for PCB data to update
    end = (df['last_x_months'] <= (last_n_months+1))
